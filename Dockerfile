FROM daktela/php-fpm:8.4

# Install PHP and other packages

RUN apk update && \
    apk upgrade --no-cache && \
    apk add --no-cache nginx git curl nodejs npm yarn sudo nano vim ncdu openssh-client
...
COPY ./config/php/php-ini-xdebug.ini /etc/php84/conf.d/50-xdebug.ini
...
RUN apk add -t buildtools g++ make autoconf php84-pear php84-dev zlib-dev libevent-dev icu-dev libidn2-dev libidn-dev zlib libevent icu libidn2 libidn curl-dev && \
    ln -s /usr/bin/pecl84 /usr/bin/pecl && \
    pecl install raphf && \
    echo extension=raphf > /etc/php84/conf.d/00_raphf.ini && \
    yes | yes | yes | yes | yes | yes | pecl install pecl_http && \
    echo extension=http.so > /etc/php84/conf.d/01_http.ini && \
    apk del buildtools
